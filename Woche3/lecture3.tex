...
\end{overprint}
\onslide<9->
State in der monadischen Form fügt einfach nur einen Funktionsparameter \texttt{s} hinzu, versteckt das \texttt{(b,s)} und gibt lediglich das \texttt{b} in der do-Notation zurück.
\end{frame}

\subsection{Functor/Applicative/Monad}
\begin{frame}[fragile]
Hilfreich ist es, sich die State-Monade als Berechnung vorzustellen, die noch nicht ausgeführt werden kann, weil der \textbf{Anfangzustand} (initial State) noch nicht bekannt ist.\bigskip \\
\pause
Man \emph{bekommt} also erst später irgendwann einen State, bearbeitet ihn ggf. und gibt dann den geänderten State weiter.\bigskip
...
\begin{frame}[fragile]
Wenn wir Muster finden, dann faktorisieren wir sie doch raus!\bigskip
...
\pause
\begin{important}
Wichtig: \texttt{IO} ist nicht additiv! Es gibt keinen IO-T!
\end{important}
\end{frame}
...
\begin{frame}[fragile]
Ein weiteres Echtwelt-Beispiel könnte etwa der folgende Aufruf sein:\bigskip
...
\begin{minted}{haskell}
mainLoop :: RWST Env () State \texttt{IO} ()
mainLoop = do
         e <- ask
